...
class ClientViewSet(viewsets.ModelViewSet):
	...
	def get_queryset(self):
		...
		tenant = self.request.user.get_tenant()
		discounts = Discount.objects.filter(tenant=tenant).order_by('discount_percentage')
		...
		if q:
			...
			return Client.objects.annotate(
				...
				Q(phone_number__icontains=q) | Q(full_name__icontains=q), discount__in=discounts
			)
